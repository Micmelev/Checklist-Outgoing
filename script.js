...
document.getElementById('subjectName').onchange = function(){
  ...
  if (name1 === "Investment Authorization") {
    form = ["Recent notes related to outgoing money", "Recent incoming money in correct account",
      "Notes in form tab", "Fees match", "Signed/Dated Form", "Do we have the asset", "For PP does investment amount match what's being invested", "Are there wiring wiring instructions in the asset and does is match", "Beneficiary name matches", "Bank Name Matches","If sending check does address match", "Check if Multi Member", "Is amount under 800K (if not needs a verbal)"]
  }
  else if (name1 === "Distribution") {
    form = ["Recent notes related to outgoing money", "Recent incoming money in correct account", "Notes in form tab", "Fees match", "Signed/Dated Form", "Beneficiary name matches","Bank Name Matches", "If sending check does address match", "Is amount under 800K", "Is there a task for account closure"]
  }
  else if (name1 === "EPR") {
    form = ["Recent notes related to outgoing money", "Recent incoming money in correct account", "Notes in form tab", "Fees match", "Signed/Dated Form", "Do we have the asset", "Beneficiary name matches","Bank Name Matches", "If sending check does address match", "Is amount under 800K(if not needs a verbal)", "Does the invoice have the correct titling and asset"]
  }
